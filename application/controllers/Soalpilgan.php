...
class Soalpilgan extends CI_Controller {
	...
	public function index()
	{
		...
		$this->load->view('pertanyaan/soalpilgan_view',$data);
	}
 
	public function kirimsoal(){
		// if($this->_validate2()){
			$mhs = json_encode($this->pertanyaanpil_model->getmahasiswa($this->input->post("kirsem"))->result());
			$temp = count($this->pertanyaanpil_model->getmahasiswa($this->input->post("kirsem"))->result());
	...
	public function ambilpil($idsoalpil){
		echo json_encode(
				$this->pertanyaanpil_model
					->ambilsoalpil($idsoalpil)->result());
	}
